...
class _DashboardPageState extends State<DashboardPage>{
  ...
  @override Widget build(BuildContext context){return RefreshIndicator(onRefresh:load,child:SafeArea(child:ListView(padding:const EdgeInsets.all(16),children:[Text(customer?'MY ACCOUNT':'AJANTA SAREE CENTRE',style:const TextStyle(fontSize:23,fontWeight:FontWeight.bold)),const Text('Satna (M.P.)'),const SizedBox(height:20),loading?const Center(child:CircularProgressIndicator()):Column(children:[card(Icons.receipt_long,"Today's Sales",sales),card(Icons.shopping_cart,"Today's Purchases",purchases),card(Icons.people,'Customer Outstanding',customerOutstanding),card(Icons.store,'Trader Outstanding',traderOutstanding)])])));}
}
